...
function isDoi(input: string): boolean {
    const t = input.trim();
    return /^10\.\d{4,}\//.test(t) || /doi\.org\//i.test(t);
}

function isArxivId(input: string): boolean {
    const t = input.trim();
    return /^\d{4}\.\d{4,5}/.test(t) || /arxiv\.org/i.test(t);
}

function extractDoi(input: string): string {
    const t = input.trim();
    const m = t.match(/doi\.org\/(10\.\d{4,}\/[^\s]+)/i);
    return m ? m[1] : t;
}

function extractArxivId(input: string): string {
    const t = input.trim();
    const m = t.match(/arxiv\.org\/abs\/(\d{4}\.\d{4,5})/i);
    if (m) return m[1];
    const m2 = t.match(/^(\d{4}\.\d{4,5})/);
    return m2 ? m2[1] : t;
}
...
async function fetchFromCrossRef(doi: string): Promise<ResolvedPaper> {
    ...
    async function fetchFromArxiv(id: string): Promise<ResolvedPaper> {
      ...
      export async function resolvePaper(input: string): Promise<ResolvedPaper> {
        ...
        throw new Error("Not a valid DOI or arXiv ID. Examples: 10.1234/foo or 2301.12345");
      }
